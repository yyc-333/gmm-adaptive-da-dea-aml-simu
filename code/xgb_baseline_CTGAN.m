...
%% =====================================================
%   XGBoost baseline for CTGAN simulation files
%   Modified version:
%   1) Keep two-layer micro summary logic of xgb_baseline_CTGAN_two_layer_micro.m
%   2) Add correlation-based feature groups and select best_group
%   3) Jointly select best_group + XGB parameters by comprehensive score
%   4) Choose the F1-optimal threshold on the training set of each fitted model
% =====================================================

dataFolder = 'D:\白血病模拟数据\Simulated_data_MC_3_3000';
resultRootFolder = 'D:\白血病模拟数据\Simulated_data_MC_3_3000\DADEA_CTGAN_100_result';
resultFolder = fullfile(resultRootFolder, 'XGB_baseline_bestgroup_f1thr_result');
...
fprintf('发现 %d 个 CSV 文件，开始 XGB-best_group-F1阈值 对照实验...\n', length(files));
...
fprintf(fid, ['filename,best_group,num_group_features,best_n_estimators,best_max_depth,best_learning_rate,best_threshold,best_CV_score,best_CV_F1,best_CV_AUC,' ...
...
for fi = 1:length(files)
    ...
    %% GMM 聚类：优先调用 GMM_Clustering；否则作为单一 cluster 处理
    ...
    for ci = 1:nClusters
        ...
        fprintf('\n开始 XGB-best_group-F1阈值：%s | train=%d, test=%d, K=%d\n', ...
        ...
        %% 联合遍历：best_group + XGB 参数
        for gi = 1:nGroups
            cols = groups_unique{gi};
            X_group = X_train_full(:, cols);

            for ti = 1:numel(nEstimatorsGrid)
                for di = 1:numel(maxDepthGrid)
                    for li = 1:numel(learningRateGrid)
                        ...
                        for k = 1:K_fold
                            ...
                            try
                                model = fit_weighted_xgb(X_group(fit_idx, :), y_train(fit_idx), ...
                                    nEstimatorsGrid(ti), maxDepthGrid(di), learningRateGrid(li));
                            catch ME
                                fprintf('  警告：%s group%d fold%d XGB拟合失败：%s\n', ...
                            ...
                            [pred_val, score_val] = predict_xgb_positive_score(model, X_group(val_idx, :));
                        ...
                        if score > bestScore
                            ...
                            bestGroup = gi;
                            bestGroupCols = cols;
                            bestParams.nEstimators = nEstimatorsGrid(ti);
                            bestParams.maxDepth = maxDepthGrid(di);
                            bestParams.learningRate = learningRateGrid(li);
                            bestCV.F1 = F1_cv;
                            bestCV.AUC = AUC_cv;
        ...
        if bestScore == -Inf || isempty(bestGroupCols)
            fprintf('跳过 %s：所有 best_group + XGB 参数组合均不可用。\n', clusterName);
            continue;
        end

        %% 使用 best_group + best XGB 参数在完整训练集重训
        X_train_best = X_train_full(:, bestGroupCols);
        X_test_best = X_test_full(:, bestGroupCols);

        finalModel = fit_weighted_xgb(X_train_best, y_train, ...
            bestParams.nEstimators, bestParams.maxDepth, bestParams.learningRate);

        [pred_train, score_train] = predict_xgb_positive_score(finalModel, X_train_best);
        ...
        [pred_test, score_test] = predict_xgb_positive_score(finalModel, X_test_best);
        ...
        fprintf('XGB-CV micro：group=%d, nFeat=%d, nEstimators=%d, maxDepth=%d, LR=%.4f, Score=%.4f\n', ...
            bestGroup, numel(bestGroupCols), bestParams.nEstimators, bestParams.maxDepth, bestParams.learningRate, bestScore);
        fprintf('XGB-CV micro：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f\n', ...
            bestCV.F1, bestCV.AUC, bestCV.precision, bestCV.recall, bestCV.accuracy);
        fprintf('XGB训练集性能：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f, threshold=%.6f\n', ...
            F1_train, AUC_train, precision_train, recall_train, accuracy_train, finalModel.threshold);
        fprintf('XGB测试集性能：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f, threshold=%.6f\n', ...
        ...
        write_metric_file(clusterFile, { ...
            'best_group', bestGroup; 'num_group_features', numel(bestGroupCols); 'best_n_estimators', bestParams.nEstimators; 'best_max_depth', bestParams.maxDepth; 'best_learning_rate', bestParams.learningRate; 'best_threshold', finalModel.threshold; 'best_CV_score', bestScore; ...
        ...
        fprintf(fid, '%s,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%s\n', ...
            clusterName, bestGroup, numel(bestGroupCols), bestParams.nEstimators, bestParams.maxDepth, bestParams.learningRate, finalModel.threshold, bestScore, ...
        ...
        save(fullfile(resultFolder, [clusterName, '_XGB_bestgroup_f1thr.mat']), ...
...
function model = fit_weighted_xgb(X, y, nEstimators, maxDepth, learningRate)
    y = double(y(:));
    X = double(X);
    ...
    try
        ...
        error('当前MATLAB Python环境未找到 xgboost/numpy。请先在MATLAB pyenv 指向的Python中安装 xgboost。');
    end
    ...
    dtrain = xgb.DMatrix(np.array(Xz), pyargs('label', np.array(double(y(:)))));
    params = py.dict(pyargs( ...
        'objective', 'binary:logistic', ...
        'eval_metric', 'logloss', ...
        'max_depth', int32(maxDepth), ...
        'eta', learningRate, ...
        'subsample', 0.90, ...
    ...
    model.xgb = xgb;
    model.np = np;
    model.booster = xgb.train(params, dtrain, int32(nEstimators));
    ...
    % 在当前训练集上搜索使 F1 最大的阈值
    scoreTrain = predict_xgb_score_only(model, X);
    model.threshold = choose_f1_threshold(y, scoreTrain);
end

function [pred, scorePos] = predict_xgb_positive_score(model, X)
    scorePos = predict_xgb_score_only(model, X);
...
function scorePos = predict_xgb_score_only(model, X)
    X = double(X);
    Xz = (X - model.mu) ./ model.sigma;
    Xz(~isfinite(Xz)) = 0;
    dtest = model.xgb.DMatrix(model.np.array(Xz));
    predPy = model.booster.predict(dtest);
...
function write_metric_file(filePath, rows)
    ...
    for i = 1:size(rows, 1)
        ...
        value = rows{i, 2};
        if isnumeric(value)
            if isfinite(value) && abs(value - round(value)) < 1e-12 && any(contains(key, {'TP', 'FP', 'FN', 'TN', 'group', 'num_', 'estimators', 'depth'}))
                fprintf(fid, '%s,%d\n', key, round(value));
            else
                fprintf(fid, '%s,%.6f\n', key, value);
            end
