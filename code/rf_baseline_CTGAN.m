...
%% =====================================================
%   RF baseline for CTGAN simulation files
%   Modified version:
...
%   3) Use TreeBagger + sample Weights, no Cost matrix
...
%   6) Jointly select best_group + best RF parameters by comprehensive score
...
resultFolder = fullfile(resultRootFolder, 'RF_baseline_bestgroup_treebagger_result');
...
useSampleWeights = true;  % TreeBagger样本权重；不使用Cost代价矩阵

numTreesGrid = [100, 200];
minLeafGrid = [1, 5];
% mtry / NumPredictorsToSample 根据每个 group 的变量数动态生成：sqrt(p), p/3, p

files = dir(fullfile(dataFolder, 'synthetic_CTGAN_*.csv'));
fprintf('发现 %d 个 CSV 文件，开始 RF-TreeBagger-best_group 对照实验...\n', length(files));
...
for fi = 1:length(files)
    ...
    for ci = 1:nClusters
        ...
        fprintf('\n开始 RF-TreeBagger：%s | train=%d, test=%d, K=%d\n', ...
        ...
        %% =====================================================
        %   联合遍历：best_group + TreeBagger RF参数
        %% =====================================================
        for gi = 1:nGroups
            cols = groups_unique{gi};
            X_group = X_train_full(:, cols);
            p_group = numel(cols);
            mtryGrid = unique([max(1, round(sqrt(p_group))), max(1, round(p_group/3)), p_group]);
            mtryGrid = mtryGrid(mtryGrid >= 1 & mtryGrid <= p_group);

            for ti = 1:numel(numTreesGrid)
                for li = 1:numel(minLeafGrid)
                    for mi = 1:numel(mtryGrid)
                        ...
                        for k = 1:K_fold
                            ...
                            try
                                model = fit_treebagger_rf(X_group(fit_idx, :), y_train(fit_idx), ...
                                    numTreesGrid(ti), minLeafGrid(li), mtryGrid(mi), useSampleWeights);
                            catch ME
                                fprintf('  警告：%s group%d fold%d TreeBagger拟合失败：%s\n', ...
                            ...
                            [~, score_val] = predict_treebagger_positive_score(model, X_group(val_idx, :));
                        ...
                        if score > bestScore
                            ...
                            bestGroup = gi;
                            bestGroupCols = cols;
                            bestParams.numTrees = numTreesGrid(ti);
                            bestParams.minLeaf = minLeafGrid(li);
                            bestParams.mtry = mtryGrid(mi);
                            bestCV.F1 = F1_cv;
                            bestCV.AUC = AUC_cv;
        ...
        if bestScore == -Inf || isempty(bestGroupCols)
            fprintf('跳过 %s：所有 best_group + RF 参数组合均不可用。\n', clusterName);
            continue;
        end

        %% 使用 best_group + best RF 参数在完整训练集重训
        X_train_best = X_train_full(:, bestGroupCols);
        X_test_best = X_test_full(:, bestGroupCols);

        finalModel = fit_treebagger_rf(X_train_best, y_train, ...
            bestParams.numTrees, bestParams.minLeaf, bestParams.mtry, useSampleWeights);

        [~, score_train] = predict_treebagger_positive_score(finalModel, X_train_best);
        ...
        [~, score_test] = predict_treebagger_positive_score(finalModel, X_test_best);
        ...
        fprintf('RF-CV micro：group=%d, nFeat=%d, Trees=%d, Leaf=%d, mtry=%d, CV_Thr=%.6f, Final_Thr=%.6f, Score=%.4f\n', ...
            bestGroup, numel(bestGroupCols), bestParams.numTrees, bestParams.minLeaf, bestParams.mtry, bestCV.threshold, finalModel.threshold, bestScore);
        fprintf('RF-CV micro：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f\n', ...
            bestCV.F1, bestCV.AUC, bestCV.precision, bestCV.recall, bestCV.accuracy);
        fprintf('RF训练集性能：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f\n', ...
            F1_train, AUC_train, precision_train, recall_train, accuracy_train);
        fprintf('RF测试集性能：F1=%.4f, AUC=%.4f, Precision=%.4f, Recall=%.4f, Accuracy=%.4f\n', ...
        ...
        write_metric_file(clusterFile, { ...
            'best_group', bestGroup; 'num_group_features', numel(bestGroupCols); 'best_num_trees', bestParams.numTrees; 'best_min_leaf', bestParams.minLeaf; 'best_mtry', bestParams.mtry; 'best_threshold', finalModel.threshold; 'best_CV_score', bestScore; ...
        ...
        fprintf(fid, '%s,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%s\n', ...
            clusterName, bestGroup, numel(bestGroupCols), bestParams.numTrees, bestParams.minLeaf, bestParams.mtry, finalModel.threshold, bestScore, ...
        ...
        save(fullfile(resultFolder, [clusterName, '_TreeBagger_bestgroup.mat']), ...
...
if ~isempty(metricFiles)
    ...
    fprintf('RF跨文件算术平均和标准差已保存到：%s\n', outputFile);
else
    fprintf('未找到 synthetic_CTGAN_*_global_micro_metrics.csv，跳过跨文件汇总。\n');
end

fprintf('\nRF-TreeBagger-best_group 对照实验完成。结果目录：%s\n', resultFolder);
...
function model = fit_treebagger_rf(X, y, numTrees, minLeafSize, mtry, useSampleWeights)
    y = double(y(:));
    X = double(X);
    ...
    if useSampleWeights
        ...
        rfModel = TreeBagger(numTrees, Xz, y_cat, ...
    ...
    model.minLeafSize = minLeafSize;
    model.mtry = mtry;
    model.useSampleWeights = useSampleWeights;
    model.posClassIdx = get_positive_class_index(rfModel);

    % 在当前训练集上选择使 F1 最大的分类阈值
    [~, score_train] = predict(rfModel, Xz);
    posCol = model.posClassIdx;
    if isempty(posCol) || posCol > size(score_train, 2)
        posCol = min(2, size(score_train, 2));
    end
    p_train = normalize_score_if_needed(score_train(:, posCol));
    model.threshold = choose_f1_threshold(y, p_train);
end

function [pred, scorePos] = predict_treebagger_positive_score(model, X)
    X = double(X);
    Xz = (X - model.mu) ./ model.sigma;
    Xz(~isfinite(Xz)) = 0;

    [pred_raw, score] = predict(model.rfModel, Xz);
    pred = convert_pred_to_double(pred_raw);

    posCol = model.posClassIdx;
    if isempty(posCol) || posCol > size(score, 2)
        posCol = min(2, size(score, 2));
    end
    scorePos = score(:, posCol);
    scorePos = normalize_score_if_needed(scorePos);
end

function posCol = get_positive_class_index(rfModel)
    ...
    posCol = find(classStrings == "1", 1);
    if isempty(posCol)
        posCol = min(2, numel(classStrings));
    end
end
